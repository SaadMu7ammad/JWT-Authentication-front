...
function All() {
  ...
  useEffect(() => {
    ...
    async function fetchTasks() {
      try {
        ...
        settemp([]);
        //   temp = [];
        const result = await axios.get('https://master--taskatt.netlify.app/all', {
          headers,
        });
  ...
  async function fetchTasks() {
    try {
      ...
      settemp([]);
      //   temp = [];
      const result = await axios.get('https://master--taskatt.netlify.app/all', { headers });
      //   console.log(result.data);
      result.data.map((item) => {
        //   console.log(item.task);
        item.task.forEach((element) => {
        ...
        settemp([...newTemp]);
        openSocket('https://master--taskatt.netlify.app');
      });
      // settemp(result.data);
    } catch (err) {
      console.log(err);
